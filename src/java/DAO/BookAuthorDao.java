...
public class BookAuthorDao extends DBContext {
    ...
    //Đém số lượng BookAuthors theo BookID
    public int countBookAuthorsByBookID(int id) {
        int count = 0;
        String sql = "SELECT COUNT(*) FROM BookAuthors WHERE BookID = ?";
        try {
            stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
            rs = stm.executeQuery();
            ...
            System.out.println("countBookAuthorsByBookID: " + e.getMessage());
        } finally {
            try {
                // Đóng ResultSet, PreparedStatement và kết nối sau khi sử dụng
                if (rs != null) {
